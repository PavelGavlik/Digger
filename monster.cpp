...
struct monster
{
	int16_t x, y, h, v, xr, yr, dir, hdir, t, hnt, death, bag, dtime, stime,
		chase;
	bool flag, nob, alive;
} mondat[6];

int16_t nextmonster = 0, totalmonsters = 0, maxmononscr = 0, nextmontime =
	0, mongaptime = 0;
int16_t chase = 0;
...
void domonsters(void)
{
	...
	{
		if (nextmonster < totalmonsters && nmononscr() < maxmononscr &&
			isalive() && !bonusmode)
			createmonster();
...
void monai(int16_t mon)
{
	...
	if (mondat[mon].xr == 0 && mondat[mon].yr == 0)
	{
		...
		if (abs(diggery(dig) - mondat[mon].y) >
			abs(diggerx(dig) - mondat[mon].x))
		{
	...
	push = true;
	drawmon(mon, mondat[mon].nob, mondat[mon].hdir, mondat[mon].x,
			mondat[mon].y);
	for (i = 0; i < TYPES; i++)
		clfirst[i] = first[i];
	for (i = 0; i < SPRITES; i++)
...
bool fieldclear(int16_t dir, int16_t x, int16_t y)
{
	...
	case DIR_DOWN:
		if (y < 9)
			if ((getfield(x, y + 1) & 0x2000) == 0)
				if ((getfield(x, y + 1) & 0x40) == 0 ||
					(getfield(x, y) & 0x800) == 0)
					return true;
	}
	return false;
}
