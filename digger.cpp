...
struct digger
{
	int16_t x, y, h, v, rx, ry, mdir, dir, bagtime, rechargetime, fx, fy, fdir,
		expsn, deathstage, deathbag, deathani, deathtime, emocttime, emn, msc,
		lives, ivt;
...
void initdigger(void)
{
	...
	for (dig = curplayer; dig < diggers + curplayer; dig++)
	{
		...
		movedrawspr(dig + FIRSTDIGGER - curplayer, digdat[dig].x,
					digdat[dig].y);
...
void updatefire(int n)
{
	...
	if (digdat[n].notfiring)
	{
		if (digdat[n].rechargetime != 0)
			...
			if (digdat[n].alive)
			{
				...
				movedrawspr(FIRSTFIREBALL + n - curplayer, digdat[n].fx,
							digdat[n].fy);
				soundfire(n);
			}
	}
	else
	{
		...
		while (i != -1)
		{
			if (i - FIRSTDIGGER + curplayer != n &&
				!digdat[i - FIRSTDIGGER + curplayer].invin &&
...
void diggerdie(int n)
{
	...
	case 2:
		...
		if (digdat[n].deathani == 0)
			music(2);
		drawdigger(n - curplayer, 14 - digdat[n].deathani, digdat[n].x,
				   digdat[n].y, false);
		for (i = 0; i < TYPES; i++)
			clfirst[i] = first[i];
		for (i = 0; i < SPRITES; i++)
		...
		else
		{
			...
			else if (isalive() && digdat[n].lives > 0)
			{
				...
				if (digdat[n].lives > 0)
				{
					...
					movedrawspr(n + FIRSTDIGGER - curplayer, digdat[n].x,
								digdat[n].y);
					digdat[n].notfiring = true;
		...
		if (digdat[n].alive)
			if (digdat[n].mdir == DIR_UP || digdat[n].mdir == DIR_DOWN)
				if ((digdat[n].x - 12) / 20 == h)
					if ((digdat[n].y - 18) / 18 == v ||
						(digdat[n].y - 18) / 18 + 1 == v)
						return true;
	return false;
}
